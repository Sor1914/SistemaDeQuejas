...
export default function MainLayout() {
  const [drawerOpen, setDrawerOpen] = useState(false); // Sidebar tipo drawer (siempre toggle)
  const [userMenuOpen, setUserMenuOpen] = useState(false);
  const [topNavOpen, setTopNavOpen] = useState(false); // Nav superior horizontal plegable
  const [grpQuejas, setGrpQuejas] = useState(true);
  const [grpReportes, setGrpReportes] = useState(false);
  const { isAuthenticated, permisos, user, logout } = useAuth();
  const [showLogin, setShowLogin] = useState(false);
  const hasAny = (p: Permisos | null, keys: (keyof Permisos)[]) =>
    !!p && keys.some((k) => p[k]);

  // Cierra menús al navegar por hash (anclas)
  useEffect(() => {
    const close = () => {
      setDrawerOpen(false);
      setTopNavOpen(false);
  ...
  return (
    ...
    >
      ...
      >
        <div className="w-full px-4 sm:px-6 lg:px-8 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative flex items-center gap-3">
              {/* Nombre del usuario cuando hay sesión */}
              {isAuthenticated && (
                <span
        ...
        {/* NAV SUPERIOR: horizontal y plegable (scroll-x en móviles) */}
        <div
          id="top-hnav"
          className={`w-full border-t transition-[max-height] duration-300 overflow-hidden ${
            topNavOpen ? "max-h-24" : "max-h-0"
          }`}
          style={{ background: "var(--surface)", borderColor: "var(--border)" }}
        >
          <nav
            className="w-full px-4 md:px-6 h-12 flex items-center gap-6 overflow-x-auto whitespace-nowrap"
      ...
      {/* DRAWER lateral (siempre toggleable) */}
      <div className="relative">
        ...
        {/* Panel */}
        <aside
          className={`fixed z-50 inset-y-0 left-0 w-[280px] transform transition-transform duration-300 ${
        ...
        >
          <nav className="p-4 space-y-3">
            ...
            {/* ---- Si NO está logueado: solo "Registrar queja" → /login ---- */}
            <Link
              to="/quejas/nueva"
            ...
            {/* ---- Si está logueado: pintar por permisos ---- */}
            {isAuthenticated && (
              <>
                {/* Grupo: Catálogos */}
                {hasAny(permisos, [
                  "CatalogoPuntosAtencion",
                  "UsuarioPuntoAtencion",
                  "CatalogoQuejas",
                ]) && (
                  <>
                    <button
                ...
                {hasAny(permisos, [
                  "AsignacionRechazo",
                  "SeguimientoCentralizador",
                  "SeguimientoPuntoAtencion",
                  "AutoConsulta",
                ]) && (
                  <>
                    <div
